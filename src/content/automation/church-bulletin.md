...
슬라이드를 만드는 일은 자료를 모으는 일과 성격이 달랐습니다. 악보 이미지를 찾아 붙이는 방식은 파일이 로컬에 있어야 하고, 교회마다 가진 악보도 다릅니다. 그래서 **가사 텍스트로 슬라이드를 만드는 방향**을 따로 떼어내 [예배 PPT 생성기](/worship-ppt)로 만들었습니다. 악보 파일 없이도 쓸 수 있어 다른 교회에도 열어둘 수 있습니다.

두 방식은 지금 병행합니다. 예원교회는 기존 악보 자산이 있어 로컬 생성을 쓰고, 그렇지 않은 곳은 생성기를 씁니다.
